# 素材

app 用到的圖全部在這裡，沒有第二個地方。

```
assets/
  bo/       慢寶本人（11 張）
  dish/     菜色（12 張，檔名對應食譜 id）
  bg/       背景質感
  source/   生圖用的定稿參考 —— app 不會載入它們
```

## bo/ — 慢寶

| 檔名 | 是什麼 | 用在哪 |
|---|---|---|
| `hang_sleep_tree` | 睡著、倒掛在有葉子的樹枝上 | 封面。**切三層動**，見下 |
| `hang_drape_tree` | 趴在有葉子的樹枝上、醒著 | 兩個入口那頁 |
| `hang_drape` | 同上但是近景、沒有樹 | 今日菜單那個 132px 的小圖 |
| `hang_u` | U 型倒掛、醒著 | 目前備用 |
| `cook_bowl` | 站著攪碗 | 目前備用 |
| `cook_pan` | 站著端鍋 | 食譜詳情（牠說話那塊） |
| `face_sleepy` `face_satisfied` `face_cheeky` `face_blank` `face_thinking` `face_surprised` | 六個表情頭像，已去背 | 底部橫條、空狀態 |

## dish/ — 菜色

`r1`–`r12`，數字就是 `recipes.js` 裡的食譜 `id`。全部 700×525 WebP。

## bg/ — 背景

`wood.webp` — 480×480 無縫貼圖，食譜牆的木牆。CSS 以 240px 重複。

## 封面那張是切三層播的

`hang_sleep_tree.webp` 在 `index.html` 裡疊三次，用 `clip-path` 各露一條橫帶：

| 層 | 範圍（於 900×559） | 動作 |
|---|---|---|
| `bo-leaf-a` | 縱 0–25.6%，橫 0–48.7% | **不動**（左邊那根分岔的枝） |
| `bo-leaf-b` | 縱 0–25.6%，橫 48.7–76.2% | `skewX` ±2°，5.6s |
| `bo-leaf-c` | 縱 0–25.6%，橫 76.2–100% | `skewX` ±3.5°，3.9s，相位 −1.2s |
| `bo-branch` | 縱 25.4–40.4% | 不動 |
| `bo-swing` | 縱 38.6% 以下 | `rotate` ±1.4°，支點 52.8% / 33.3% |

**要換這張圖的話，每個百分比都要重算。** 三個條件：

1. 中間必須有一條「只有主樹枝、沒有任何東西跨過去」的乾淨橫帶
2. **慢寶層的上緣要落在主樹枝最低點以下** —— 否則牠一轉就把樹枝末端一起帶下去，
   從靜止樹枝的下緣露出來（踩過，左端出現一小截斷枝）
3. **樹枝帶的下緣要比慢寶層的上緣再低 10px 以上** —— 慢寶層的裁切邊也會跟著轉，
   離支點最遠的身體位移約 7px，留不夠會裂出一條縫

橫向的切線要落在枝葉叢之間的空白處，而且**分岔的枝要整叢在同一片裡**（不然會在分岔處撕開）。
⛔ 不要讓所有枝葉一起晃，看起來會很機械。

⛔ 不要用 CSS 另外疊葉子或樹枝當裝飾。試過，看起來就是元素拼貼。

## source/ — 生圖參考（不是 app 素材）

`bo_master.png` — **角色的唯一真相**。每次要生新的慢寶圖，都要附上它，
否則畫出來的會一次比一次偏。詳見 `manbao` 技能。

---

## ⚠️ 換圖之後一定要做的事

`index.html` 裡有一行 `const ASSETS = 'N'`。**把 N 加一。**

所有素材網址都會帶上 `?v=N`。不改的話，網址沒變，瀏覽器會繼續用舊的快取，
你換了圖也看不到。（踩過不只一次。）
